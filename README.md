...
## 구현 목록
- [X] 지하철 길찾기 제목을 보여준다.
- [X] 출발역과 도착역을 입력할 수 있는 화면을 보여준다.
  - [] 출발역과 도착역은 2글자 이상이어야 한다.
  - [] 존재하지 않는 역을 입력할 수 없다.
  - [] 출발역과 도착역이 같을 수 없다.'
  - [] 출발역과 도착역이 연결되어 있지 않으면 경로를 조회할 수 없다.
- [X] 최단거리, 최소시간 옵션을 선택할 수 있다.
  - [X] 기본값을 최단거리로 설정한다.
- [X] 길찾기 버튼을 누를 수 있다.
- [X] 결과 화면의 title을 최단거리, 최소시간 선택에 따라 다르게 볼 수 있다.
- [] 결과는 테이블 형태로 보여준다.
  - [X] 재사용 가능한 테이블 컴포넌트를 만든다.
  - [X] 결과 테이블의 테이블 header를 화면에서 볼 수 있다.
  - [] 결과값을 테이블의 body에서 볼 수 있다.
- [] 초기 데이터 설정을 한다.
- [] 데이터를 주어진 다익스트라 라이브러리와 결합한다.