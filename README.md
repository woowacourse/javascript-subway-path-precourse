...
```
- 주어진 요구사항대로 지하철역, 노선, 노선의 역을 초기화해야 한다
- 출발역과 도착역을 입력받는다
- 입력한 역명이 2글자 이상인지 검증해야 한다
- 입력한 역명이 서로 다른지 검증해야 한다
- 입력한 역명이 유효한 역명인지 검증해야 한다
- 입력한 역명이 서로 연결되어있는지 검증해야한다
- 최단거리 or 최소시간 중 어느것을 선택했는지 확인해야 한다
- 노드, 간선 정보를 입력해 다익스트라 노드에 추가한다
- 거리 기준 다익스트라를 생성한다
- 시간 기준 다익스트라를 생성한다
- 주어진 역들 정보로 거리의 총합을 구한다
- 주어진 역들 정보로 시간의 총합을 구한다
- 계산한 거리, 시간, 경로를 화면상에 출력한다
```
