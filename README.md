...
- 역의 이름은 2글자 이상으로 작성한다.
- 존재하지 않는 역은 작성할 수 없다.
- 출발역과 도착역의 이름은 달라야 한다.
- 출발역과 도착역 둘 다 작성해야 한다.
- 프로그램이 수행이 안 될 시 `alert` 메시지를 출력한다.
- 최단거리 클릭 후, 길 찾기 버튼을 누르면 목표지까지의 최단거리, 소요시간, 가는 루트를 출력한다.
- 최소시간 클릭 후, 길 찾기 버튼을 누르면 목표지까지의 거리, 최소소요시간, 가는 루트를 출력한다.
