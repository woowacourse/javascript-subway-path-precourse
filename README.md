...
> 지하철 노선도 경로 조회 미션은 우아한 테크코스 프리코스 코딩테스트입니다.
> 지하철 노선도 미션에서는 유저가 이미 등록된 지하철 역들을 통해 경로를 조회할 수 있습니다.
...
- 필요한 데이터(역, 노선, 구간 데이터)를 초기 설정하는 기능
  - [초기 설정](#초기-설정)을 참고
- 유저로부터 지하철 출발역을 입력받는 기능
- 유저로부터 지하철 도착역을 입력받는 기능

  - 🚦 예외상황
    - 유저가 존재하지 않는 역을 출발/도착역으로 입력했을 때
    - 유저가 출발/도착역을 1글자 이하로 입력했을 때
    - 유저가 출발/도착역을 같게 입력했을 ㄸ대
    - 유저가 연결되지 않은 출발/도착역을 입력했을 때
    - 유저가 앞뒤로 공백을 포함해 출발/도착역을 입력했을 때

- 유저가 선택한 최단거리/최소시간 옵션을 받는 기능
- 결과를 표로 보여주는 기능
