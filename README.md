...
## 구현할 기능 목록
- [x] 경로 조회 화면 UI 구현
- [x] 출발역, 도착역 입력 받는 기능 구현
- [x] 다익스트라 알고리즘을 이용하여, 경로 조회 시 총 거리, 총 소요 시간을 함께 출력
- [x] 경로 조회 시 *최단거리* 또는 *최소시간* 옵션을 선택할 수 있게 해야한다.
- [x] data 형식 정의
- [x] 길 찾기 버튼 클릭 시, 결과 영역 초기화
- [x] 결과 출력 시, table로 출력
- [x] 라디오 값 초기화 기능
- [x] style 적용
- 예외처리
  - [x] 출발역과 도착역은 2글자 이상이어야한다.
  - [x] 존재하지 않는 역을 출발/도착역으로 입력할 수 없다.
  - [x] 출발역과 도착역은 같을 수 없다.
  - [x] 경로에서 출발역과 도착역이 연결되지 않으면 경로를 조회할 수 없다.
  - [ ] 역 입력 시, 공백여부 상관없이 판단
