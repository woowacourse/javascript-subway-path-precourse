...
- 상태만을 따로 관리하는 파일을 만들어서 상태를 관리한다.
- 길 찾기 버튼이 눌리면 출발역과 도착역 input의 유효성(값들이 실제로 존재하는 역인지, 출발역과 도착역이 같은 역이 아닌지)을 확인한다.
- input이 유효성 검사를 통과하면 최단거리/최소시간을 기준으로 결과를 구한다.
- 결과가 구해지면 table을 만들어서 그 안에 알맞게 넣어 출력한다.
- table요소들의 테두리를 1px solid black으로 설정한다.
- 모든 문자열을 상수 처리하여 한 파일로 보관한다.
