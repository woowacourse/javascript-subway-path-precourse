...
### 세부 기능 목록
1. 페이지 로드 시 역, 노선, 구간 정보를 초기화한다.
    - 각 정보를 별도의 파일로 관리
    - 지하철 노선도에 대한 객체를 생성해 데이터를 초기화

2. 출발역과 도착역을 입력받는다.
    - 2글자 이상
    - 입력된 역이 존재하는지 조회
    - 출발역과 도착역이 같을 수 없다.
    - 출발역과 도착역이 연결될 수 있는지 확인해야한다.
    - 입력값 앞뒤 공백은 무시한다.

3. 최단거리 혹은 최소시간에 대한 경로를 결과로 보여준다.

4. 결과로 주어진 경로 상의 총 거리와 총 소요시간을 보여준다.

5. 경로 재조회 시 입력값이 유효하지 않으면 결과 테이블을 지운다.
