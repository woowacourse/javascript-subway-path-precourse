...
## 🖋 구현할 기능 목록
#### 역 데이터
- [ ] 역 데이터를 만들어서 기본 데이터 초기화 하기
#### 인풋
- [ ] 출발역, 도착역의 입력을 받아야한다
  - 예외처리는 `alert`로 알려준다.
  - [ ] 예외: 출발역과 도착역은 2글자 이상이다.
  - [ ] 예외: 출발역과 도착역이 같을 수 없다.
  - [ ] 예외: 출발역과 도착역이 연결된 노드가 아닐 경우 경로를 조회할 수 없다
  - [ ] 예외: 노선에 이미 등록된 역만 입력 받을 수 있다
- [ ] 최단거리, 최소거리 라디오 입력을 받아야 한다
#### 아웃풋
- [ ] 초기 지하철 길찾기 화면을 만들어 보여줘야 한다
- [ ] 결과 테이블 만들기을 만들어서 보여줘야 한다
  - [ ] 새로운 값이 입력될때 마다 새로운 테이블을 만들어서 새롭게 보여줘야 한다
#### 최단거리
- [ ] 최단 경로 라이브러리를 이용해서 노드와 거리 정보를 받은 후 최단 거리를 구한다.
#### 최소시간
- [ ] 최단 경로 라이브러리를 이용해서 노드와 시간 정보를 받은 후 최소 시간을 구한다.
