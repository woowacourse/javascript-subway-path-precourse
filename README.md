...
- 초기 설정
  ...
  - 지하철역으로 교대, 강남, 역삼, 남부터미널, 양재, 양재시민의숲, 매봉 역 정보를 등록한다.
  - 노선으로 2호선, 3호선, 신분당선을 등록한다.
  - 각 노선에 아래와 같이 역과 구간 정보를 등록한다.
...
- 경로 조회
  ...
  - 출발역/도착역 input, 최단거리/최단시간 radio button, 길찾기 button을 생성한다.
  - 출발역과 도착역을 입력받는다.
  - 최단거리, 최소시간을 선택할 수 있게 한다.
  - 길찾기 버튼을 누르면 결과 창을 띄운다.
  - 최단거리를 계산한다.
  - 최소시간을 계산한다.
  - 총 거리를 계산한다.
  - 총 소요시간을 계산한다.
  - 최단거리, 최소시간 선택에 맞추어 총 거리, 총 소요시간을 띄운다.

- 예외 처리

  - 출발역과 도착역에 존재하지 않는 역은 입력할 수 없다.
  - 출발역과 도착역에 공백을 입력할 수 없다.
  - 출발역과 도착역에 2글자 미만을 입력할 수 없다.
  - 출발역과 도착역이 같을 수 없다.
  - 예외상황을 입력할 경우 결과창을 띄우지 않는다.
  - ~~최단거리와 최소시간을 선택하지 않을 수 없다. (디폴트 값이 있기 때문에 불필요)~~
  - 출발역과 도착역이 연결되지 않으면 조회할 수 없다.

- 리팩토링
  - 데이터 초기화
  - 자바스크립트 컨벤션에 맞추어 리팩토링
  - 상수처리
