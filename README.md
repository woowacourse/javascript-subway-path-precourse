...
### 화면 레이아웃
- 메인화면에 제목을 표시한다
- 메인화면에 출발역과 입력창을 표시한다
- 메인화면에 도착역과 입력창을 표시한다
- 메인화면에 최단거리/ 최소시간 라디오버튼을 표시한다
- 메인화면에 길찾기 버튼을 표시한다
- 메인화면에 사용자가 입력한 조건문의 결과를 테이블로 표시한다

### 경로 조회 기능
- 출발역과 도착역을 입력받는다
  - 예외: 출발역과 도착역을 입력하지 않았으면 예외가 발생한다
  - 예외: 출발역과 도착역이 같으면 예외가 발생한다
  - 예외: 출발역과 도착역이 없는 역이면 예외가 발생한다
- 경로 조회 옵션을 선택한다 (최단거리 / 최소시간)
  - 예외: 라디오버튼의 이름을 눌러서도 선택할 수 있다


### 경로 계산 기능
- (출발역, 도착역)의 최단거리를 계산한다
- (출발역, 도착역)의 최단시간을 계산한다
