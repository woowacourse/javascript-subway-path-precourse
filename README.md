...
- 시작시점에 데이터를 초기화시켜야한다.
- 출발역 input에서 출발역을 입력받는다.
  - 예외 1 : 출발역은 2글자 이상이어야한다.
  - 예외 2 : 출발역은 존재하는 역이어야한다.
- 도착역 input에서 도착역을 입력받는다.
   - 예외 1:  도착역은 2글자 이상이어야한다.
   - 예외 2 : 도착역은 존재하는 역이어야한다.

- 최단거리, 최소시간에 따라서 아래 나오는 결과가 다르다.

- 길찾기 버튼을 눌러 결과를 조회한다.
  - 예외 1 : 출발역과 도착역은 같을 수 없다.
  - 예외 2 : 출발역과 도착역이 연결되지 않으면 경로를 조회할 수 없다.
  - 예외 3 : 출발역과 도착역을 반대로 쓰더라도 '구간'이기때문에, 총 거리와 총 소요시간이 바르게 출력되어야한다.

- 총 시간과 총 거리를 계산한다.
- 총 시간은 분, 총 거리는 km로 표현한다.
