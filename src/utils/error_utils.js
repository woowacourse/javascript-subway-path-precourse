...
export default class ErrorUtils {
  setVariables() {
    ...
    this.departureInput = this._commonUtils.getById(inputConsts.DEPARTURE_INPUT_IDNAME).value;
    this.arrivalInput = this._commonUtils.getById(inputConsts.ARRIVAL_INPUT_IDNAME).value;
  }

  inputErrorExists() {
    this.setVariables();

    if (this.doesNotExist(this.departureInput) + this.doesNotExist(this.arrivalInput)) {
  ...
  alertCorrespondingError() {
    if (this.doesNotExist(this.departureInput)) {
      this._commonView.alertError(inputConsts.DEPARTURE_DOES_NOT_EXIST_ERROR_MESSAGE);
    }

    if (this.doesNotExist(this.arrivalInput)) {
      this._commonView.alertError(inputConsts.ARRIVAL_DOES_NOT_EXIST_ERROR_MESSAGE);
    }

    if (this.sameDepartureArrival()) {
      this._commonView.alertError(inputConsts.SAME_DEPARTURE_ARRIVAL_ERROR_MESSAGE);
    }
  ...
  notConnected(departure, arrival) {
    if (this.notSameLine(line2호선, departure, arrival) && this.notSameLine(line3호선, departure, arrival) && this.notSameLine(line신분당선, departure, arrival)) {
  ...
  notSameLine(line, departure, arrival) {
    if (line.findIndex(station => station.name === departure) >= 0 && line.findIndex(station => station.name === arrival) >= 0) {
      return commonConsts.IS_FALSE;
    }
  ...
  cannotTransfer() {
    ...
    const arrivalLineIndex = this.findWhereArrivalIs(lineList, departureLineList);
  ...
  findWhereDepartureIs(lineList) {
    const departureLineList = [];

    lineList.forEach(line => {
      if (line.findIndex(station => station.name === this.departureInput) >= 0) {
