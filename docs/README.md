...
- Model에서 필요한 기능을 한다.
  - 역과 노선 데이터를 초기화하여 저장한다. ok
  - 출발역과 도착역을 저장하고 관리한다. ok
  - 다익스트라 객체에 데이터를 추가한다. ok
  - 다익스트라를 활용한 최단거리,최소시간 루트를 구하는 함수를 작성하고 결과 값을 리턴할 수 있게한다. ok
  - 소요되는 최단거리 수치, 최소시간 수치를 계산하여 리턴할 수 있게한다. ok (리팩토링 시급)
